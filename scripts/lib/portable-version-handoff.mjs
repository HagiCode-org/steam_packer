...
export function validatePortableVersionHandoff(plan, { planPath = '[inline]' } = {}) {
  requireObject(plan, 'handoff plan');

  const handoff = requireObject(plan.handoff, 'handoff');
  if (handoff.schema !== PORTABLE_VERSION_HANDOFF_SCHEMA) {
    throw new Error(
      `handoff.schema must be ${PORTABLE_VERSION_HANDOFF_SCHEMA}; received ${JSON.stringify(handoff.schema)} from ${planPath}.`
    );
  }
...
export async function loadPortableVersionHandoff(planPath) {
  const resolvedPlanPath = path.resolve(planPath);
  const plan = await readJson(resolvedPlanPath);
  return validatePortableVersionHandoff(plan, { planPath: resolvedPlanPath });
}
